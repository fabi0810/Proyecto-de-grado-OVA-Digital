...
import { algebraQuestionGenerator } from '../../utils/algebraQuestionGenerator'

const BoolenaExamModule = () => {
  ...
  const generateFallbackExam = (count) => {
    const questions = []
    const topics = ['laws', 'simplification', 'canonical', 'karnaugh']
    ...
    for (let i = 0; i < count; i++) {
      ...
      questions.push({
        ...
        hint: `Pista para la pregunta ${i + 1}`,
        generated: true,
        fallback: true
      })
    }
  ...
  const finishExam = () => {
    ...
    try {
      algebraQuestionGenerator.updateUserProfile(results)
    } catch (error) {
        ...
        <div className="bg-gray-50 border border-gray-200 rounded-lg p-6">
          ...
          <div className="grid md:grid-cols-2 gap-3 text-sm">
            <div className="flex items-center space-x-2">
              ...
              <span className="text-gray-700">Leyes del Álgebra de Boole</span>
            </div>
            <div className="flex items-center space-x-2">
              <span className="text-purple-600">🔄</span>
              <span className="text-gray-700">Simplificación de expresiones booleanas</span>
            </div>
            <div className="flex items-center space-x-2">
              <span className="text-green-600">📐</span>
              <span className="text-gray-700">Formas canónicas (SOP y POS)</span>
            </div>
            <div className="flex items-center space-x-2">
              <span className="text-orange-600">🗺️</span>
              <span className="text-gray-700">Mapas de Karnaugh</span>
            </div>
          </div>
        </div>
      </div>
            ...
            <div className="flex-1">
              <div className="flex items-center space-x-3 mb-4">
                <span className={`px-3 py-1 rounded-full text-xs font-semibold ${
                  currentQ?.type === 'laws' ? 'bg-blue-100 text-blue-800' :
                  currentQ?.type === 'simplification' ? 'bg-purple-100 text-purple-800' :
                  currentQ?.type === 'canonical' ? 'bg-green-100 text-green-800' :
                  'bg-orange-100 text-orange-800'
                }`}>
                  {currentQ?.type === 'laws' ? '⚖️ Leyes' :
                   currentQ?.type === 'simplification' ? '🔄 Simplificación' :
                   currentQ?.type === 'canonical' ? '📐 Formas Canónicas' : '🗺️ Karnaugh'}
                </span>
                <span className={`px-3 py-1 rounded-full text-xs font-semibold ${
                    ...
                    <div className="flex-1">
                      <div className="flex items-center space-x-2 mb-2">
                        ...
                        <span className={`px-2 py-1 rounded text-xs font-semibold ${
                          question.type === 'laws' ? 'bg-blue-100 text-blue-800' :
                          question.type === 'simplification' ? 'bg-purple-100 text-purple-800' :
                          question.type === 'canonical' ? 'bg-green-100 text-green-800' :
                          'bg-orange-100 text-orange-800'
                        }`}>
                          {question.type === 'laws' ? 'Leyes' :
                           question.type === 'simplification' ? 'Simplificación' :
                           question.type === 'canonical' ? 'Canónicas' : 'Karnaugh'}
                        </span>
                      </div>
...
export default BoolenaExamModule